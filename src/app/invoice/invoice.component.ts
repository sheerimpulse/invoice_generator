...
export class InvoiceComponent implements OnInit {
  ...
  generatePDF() {
    ...
    //COLUMNS USED IN FAST DELIVERY
    for (let spec of retrievedSpecifications[0]){
      columnElement1.push({
        text:spec.date+', '+spec.time+' '+spec.orderNumber+' '+spec.price+' *',
        fontSize:40,
        margin:[170,0,0,0],
        lineHeight:2,
      })
    }
    for (let spec of retrievedSpecifications[1]){
      columnElement2.push({
        text:spec.date+', '+spec.time+' '+spec.orderNumber+' '+spec.price+' *',
        fontSize:40,
        margin:[0,0,170,0],
    ...
    //COLUMNS USED IN NORMAL DELIVERY
    for (let spec of retrievedSpecifications[0]){
      columnElement3.push({
        text:spec.date+', '+spec.time+' '+spec.orderNumber+' '+spec.price+' *',
        fontSize:40,
    ...
    const docDefinition = {
      ...
      pageMargins: [0, 360, 0, 160],
      header:{image:'headerbanner'},
      footer:function(currentPage:any,pageCount:any){
        return {text:'Page | '+currentPage, fontSize:40, alignment:'right', margin:[0,0,170,0]};
      },
      content: [
        // {
        //   image: 'headerbanner',
        ...
        { text: 'Invoice', margin:[0,49.5,0,0], alignment:'center', fontSize:'98', bold:true},
        {
          //CLIENT DETAILS
          columns: [
            [
       ...
       
        {
          columns:[
            [{text:'Date                              #              €',
            fontSize:40, margin:[170,60.5,0,60], bold:true},
            columnElement1],
            [{text:'Date                              #              €',fontSize:40, margin:[0,60.5,0,60], bold:true},columnElement2]
          ]
        },
...

        {
          columns:[
            ...
            fontSize:40, margin:[170,60.5,0,60], bold:true},
            columnElement3],
            [{text:'Date                              #              €',fontSize:40, margin:[0,60.5,0,60], bold:true},]
          ]
        },
